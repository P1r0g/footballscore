...
public record ErrorResponse(
        ...
        @Schema(description = "Детальное описание конкретного случая ошибки", example = "Матч с id=42 не существует")
        String detail,

        @Schema(description = "URI запроса, приведшего к ошибке", example = "/api/matches/42")
        String instance,
    ...
    public record FieldError(
            ...
            @Schema(description = "Имя невалидного поля", example = "name")
            String field,

            @Schema(description = "Значение, которое было отклонено", example = "")
            Object rejectedValue,

            @Schema(description = "Причина отклонения", example = "Имя команды не может быть пустым")
            String message
    ) {}
}
