...
public class MatchService {
    ...
    public MatchResponse createMatch(MatchRequest request) {
        ...
        TeamResponse homeTeam =
                teamService.findById(request.homeTeamId());

        TeamResponse awayTeam =
                teamService.findById(request.awayTeamId());
    ...
    public MatchResponse addGoal(
        ...
        // Команду теперь получаем через TeamService,
        // а не напрямую из storage.teams.
        TeamResponse team =
                teamService.findById(request.teamId());

        eventPublisher.publishGoalScored(
                updated,
                team.getName()
        );
