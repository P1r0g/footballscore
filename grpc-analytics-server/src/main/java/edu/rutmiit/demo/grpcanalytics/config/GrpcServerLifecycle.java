...
import edu.rutmiit.demo.grpcanalytics.service.MatchAnalyticsServiceImpl;
...
public class GrpcServerLifecycle implements SmartLifecycle {
    ...
    public void start() {
        try {
            ...
            log.info("Сервис: MatchAnalytics.AnalyzeMatch()");

        } catch (IOException e) {
            throw new RuntimeException(
                    "Не удалось запустить gRPC-сервер на порту "
                            + grpcPort,
                    e
            );
        }
